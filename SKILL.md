...
### Step 1: 结构扫描
   ```powershell
   Get-ChildItem -Path D:\KnowledgeBase -Recurse -File | Measure-Object | Select-Object Count
   Get-ChildItem -Path D:\KnowledgeBase -File | Where-Object { ---
name: knowledge-base-health
description: |
  知识库健康巡检与自动维护。检测去重候选、孤点文件、INDEX.md 过期、交叉引用断裂，输出修复清单。
